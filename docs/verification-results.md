...
**已於 2026-08-04 實機確認**：兩項都滿足之後（完整尺寸組 + 重開機），
通知中心顯示正確的 App 圖示。只做其中一項都無效。
...
## 通知準時性：實測結果（2026-08-04）

先前回報「提醒有時準時、有時慢約 10 秒、偶爾沒跳」。加上設定頁的
「通知送達紀錄」實際量測 6 筆之後：
...
**通知沒有延遲問題。** 「早 1 秒」來自 `NotificationService.schedule` 用
`Int(...rounded(.down))` 無條件捨去間隔秒數——435.8 秒排成 435 秒，
所以會早約 0.8 秒送達。方向是刻意的：寧可早一點，不要晚。

那先前為什麼會覺得慢 10 秒？因為當時是**拿凍結的動態島當碼錶**。
Live Activity 因為 `ContentState` 欄位改名而解碼失敗，卡片停在最後一幀不再更新，
於是準時的通知配上停住的倒數，看起來就像遲到。詳見該次修正的 commit。

教訓：症狀被描述成「A 慢了」時，要先確認**用來判斷 A 的那把尺**是準的。
這次量錯了方向，往通知子系統查了好幾輪，而問題在顯示層。
